...
/////Lazy Update
    ...
    void join_sbt_down_kfv(int p, int x, int* fv, int* fvd, int fvSize){
        //if(p == 152770) printf("join_sbt p, key, a.size(): %d %d %d\n", p, x, fvSize);
        int pin = 1;
        ...
        if (iter[0] != 0){
            ...
            //cout<<"current_distance[t]: "<<current_distance[t]<<" t: "<<t<<endl;
        }
        //cout<<"finish"<<endl;
        ...
        for (int i = 0; i < int( RESERVE_TIME * M_K ); i++){
            if (!Q.empty()){
                ...
                //if(p == 152770) cout<<"org_t: "<<org_t<<endl;
                if (a[pt.x] == p){
                    b.push_back(OSS[a[pt.x]].a[iter[pt.x]].key);
                    //cout<<"b: "<<OSS[a[pt.x]].a[iter[pt.x]].key<<endl;
                }else{
                    b.push_back(OSS_global[a[pt.x]].a[iter[pt.x]].key);
                    //cout<<"b: "<<OSS_global[a[pt.x]].a[iter[pt.x]].key<<endl;
                }
                int t;
                while (iter[pt.x] != 0){
                    down_cand_time++;
    ...
    void initialize_object_ori(){
        ...
        if(strcmp(insert_type, "base") == 0){
            dfs_up_pro(root);
            dfs_down_pro(root);
        }else if(strcmp(insert_type, "kvc") == 0){
            dfs_up(root);
            //cout<<"finish up"<<endl;
            dfs_down(root);
